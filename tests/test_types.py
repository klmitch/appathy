...
class RegisterTypes(tests.TestCase):
    def setUp(self):
        super(RegisterTypes, self).setUp()

        self.stubs.Set(types, "media_types", {})
        self.stubs.Set(types, "type_names", {})
    ...
    def test_register_types(self):
        ...
        self.assertEqual(types.type_names, dict(foo=set([
                        "foo/bar", "application/x-foo-bar"])))
...
class TranslatorsClass(tests.TestCase):
    def setUp(self):
        super(TranslatorsClass, self).setUp()
        ...
        self.stubs.Set(types, 'type_names', dict(
